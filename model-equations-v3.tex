...
\end{equation}
where $\beta > 0$ controls response steepness. (We later use $\sigma(x) = 1/(1+e^{-x})$ for diversion dynamics; the distinction is that $S$ has adjustable slope $\beta$ for calibrating creator sensitivity, while $\sigma$ has unit slope with sensitivity absorbed into $k_A, k_\theta$.)

\textbf{Implementation note:} The simulator uses $\beta = 15$ by default. This makes the IR constraint binding: when $\pi_c < \bar{u}$, creators decisively exit (C* can fall to $\approx 0.10$), enabling clean differentiation between ``collapse'' and ``coexistence'' scenarios. Lower $\beta$ (e.g., $\beta = 1$) makes participation barely responsive to profitability, collapsing all scenarios to similar steady states.
...
\begin{remark}
The $\max(\cdot, 0)$ terms introduce non-differentiability at $C = C_{crit}$, making the system piecewise-smooth. For simulation this is unproblematic; for rigorous stability analysis, these can be replaced with softplus approximations.
\end{remark}
...
High-quality content is incentive compatible when the subscription/patronage premium exceeds additional costs:
\begin{equation}
\text{IC}: \quad p_{sub} \cdot \Delta Q \geq \Delta c
\end{equation}
where $\Delta Q$ is the quality increment from low to high, and $\Delta c = c_{high} - c_{low}$ is the \textbf{incremental cost of quality}---the extra investment required for research, fact-checking, editing, and original reporting versus quick, low-effort content production.
\end{definition}
...
$\gamma$ & Data advantage strength & AI quality dependence & dimensionless \\
...
$\Delta c$ & Incremental cost of quality & IC threshold & \$ \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
    ...
    \item \textbf{Low-Quality Trap:} $C^* \approx C_{crit}$, $Q^*$ settles at a low level. Creators survive but quality erodes.
...
\hrule
\vspace{0.5em}
\textit{Version 3.0. Feedback welcome.}
