...
\begin{equation}
\boxed{
\frac{dQ}{dt} = (1-Q) \cdot \nu \cdot R(\pi_c) + \epsilon \cdot (C - C_{crit}) - Q \cdot \lambda_0
}
\end{equation}

\textbf{Interpretation:}
\begin{itemize}
    \item $(1-Q) \cdot \nu \cdot R(\pi_c)$: Quality improves when creators can reinvest (bounded by $(1-Q)$)
    \item $\epsilon \cdot (C - C_{crit})$: Quality requires critical mass of creators; below $C_{crit}$, this term is negative
    \item $Q \cdot \lambda_0$: Baseline quality decay without active maintenance
\end{itemize}
...
\begin{equation}
\text{IC}: \quad \theta \cdot p_L \cdot A \cdot \gamma + p_{sub} \geq \Delta c
\end{equation}
where $\Delta c = c_{high} - c_{low}$ is the cost difference between high and low quality production.
\end{definition}

When IC is violated, the reinvestment signal $R(\pi_c)$ is low, and quality improvement stalls or reverses.
...
\begin{equation}
S(\pi_c^* - \bar{u}) = S(\bar{u} - \pi_c^*) \quad \Rightarrow \quad \pi_c^* = \bar{u}
\end{equation}
At interior equilibrium, creator revenue equals the outside option.

\textbf{AI equilibrium:} Setting $dA/dt = 0$:
\begin{equation}
A^* = \frac{I + r_A A^* (1 + \gamma Q^*)}{I + r_A A^* (1 + \gamma Q^*) + \delta_A}
\end{equation}
This is an implicit equation; for small $I$, we get approximately:
\begin{equation}
A^* \approx \sqrt{\frac{I}{\delta_A}} \cdot \sqrt{1 + \gamma Q^*}
\end{equation}
